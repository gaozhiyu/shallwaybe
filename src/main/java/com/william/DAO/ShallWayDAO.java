...
public class ShallWayDAO {
	...
	public ShallWayOutDTO[] readShallWay(ShallWaySearchDTO shallWaySearchDTO) throws ParseException, SQLException {
		...
		try {
			...
			// crit.setFirstResult((Integer.parseInt(shallWaySearchDTO.getPageNumber())
			// - 1) * pageSize); //removed on 27.11.2016 in order to add
			// sequenceNo.
			...
			// @SuppressWarnings("unchecked")
			// List<ShallWayOutDTO> shallWayList =
			...
			// sdfd.parse(shallWaySearchDTO.getEndTime())),
			// Restrictions.ge("endTime",
			// sdfd.parse(shallWaySearchDTO.getStartTime()))
			// ))
			// .add(Restrictions.disjunction()
			// .add(Restrictions.eq("carPool",
			// Boolean.parseBoolean(shallWaySearchDTO.getCarPool())))
			// .add(Restrictions.eq("freeTour",
			// Boolean.parseBoolean(shallWaySearchDTO.getFreeTour())))
			// .add(Restrictions.eq("hotelShare",
			// Boolean.parseBoolean(shallWaySearchDTO.getHotelShare())))
			// .add(Restrictions.eq("freeGuide",
			// Boolean.parseBoolean(shallWaySearchDTO.getFreeGuide())))
			// )
			// .list();

			crit.add(Restrictions.eq("deleteStatus", false)); // by Huzong,
																// updated on
																// 07.08.2016
			...
			crit.add(Restrictions.and(Restrictions.le("startTime", sdfd.parse(shallWaySearchDTO.getEndTime())),
	...
	public boolean updateShallWay(ShallWayUpdateDTO shallWayUpdateDTO) throws ParseException {
		...
		try {
			...
			if (shallWayUpdateDTO.getDescriptionStr() != null
					&& !"".equals(shallWayUpdateDTO.getDescriptionStr().trim())) {
